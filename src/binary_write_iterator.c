...
#include<stdio.h>
binary_write_iterator* get_new_binary_write_iterator(void* tupl, tuple_def* tpl_d, positional_accessor inline_accessor, uint32_t bytes_to_be_written_to_prefix, const worm_tuple_defs* wtd_p, const page_access_methods* pam_p, const page_modification_methods* pmm_p)
{
	...
	bwi_p->bytes_to_be_written_to_prefix = bytes_to_be_written_to_prefix; printf("=>%u\n", bwi_p->bytes_to_be_written_to_prefix);
...
uint32_t append_to_binary_write_iterator(binary_write_iterator* bwi_p, const char* data, uint32_t data_size, const void* transaction_id, int* abort_error)
{
	...
	// if the attribute is NULL, set it to EMPTY_USER_VALUE
	{
		...
		user_value attr;
		get_value_from_element_from_tuple(&attr, bwi_p->tpl_d, child_relative_accessor.exact, bwi_p->tupl);
	...
	while(data_size > 0)
	{
		...
		if(bwi_p->bytes_to_be_written_to_prefix > bwi_p->bytes_written_to_prefix)
		{
			...
			{
				point_to_prefix_character(&child_relative_accessor, old_element_count + i, bwi_p->is_inline);
