...
{
	datum_binary_reader_interface_context* cntxt = bri_p->context;
...
uint32_t read_bytes_as_stream_for_datum_binary_reader_interface(const binary_reader_interface* bri_p, char* data, uint32_t data_size, int* error)
{
	...
	data_size = min(data_size, cntxt->uval.string_or_binary_size - cntxt->bytes_read);
	if(data != NULL) // allows you to skip bytes
		memory_move(data, cntxt->uval.string_or_binary_value + cntxt->bytes_read, data_size);
...
const char* peek_bytes_as_stream_for_datum_binary_reader_interface(const binary_reader_interface* bri_p, uint32_t* data_size, int* error)
{
	...
	(*data_size) = (cntxt->uval.string_or_binary_size - cntxt->bytes_read);
	const char* data = (cntxt->uval.string_or_binary_value + cntxt->bytes_read);
	return data;
}
