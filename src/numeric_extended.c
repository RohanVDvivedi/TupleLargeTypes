...
{
	if( (dti_p->type == TUPLE) &&
		is_prefix_of_dstring(&get_dstring_pointing_to_cstring(dti_p->type_name), &get_dstring_pointing_to_literal_cstring(NUMERIC_TYPE_PREFIX)) &&
...
data_type_info* get_numeric_inline_type_info(uint32_t max_size)
{
	...
	initialize_tuple_data_type_info(dti_p, NUMERIC_TYPE_PREFIX "_" INLINE_TYPE_SUFFIX, 1, max_size, 3);
...
data_type_info* get_numeric_extended_type_info(const char* extension_sub_type, uint32_t max_size, const data_type_info* numeric_inline_p, const page_access_specs* pas_p)
{
	...
	if(extension_sub_type != NULL)
		sprintf(dti_p->type_name, NUMERIC_TYPE_PREFIX "_{%s}_" EXTENDED_TYPE_SUFFIX, extension_sub_type);
...
int compare_numeric_prefix_no_digits(numeric_sign_bits s1, int16_t e1, numeric_sign_bits s2, int16_t e2, int* digits_requirement)
{
	...
	if(IS_NEGATIVE_SIGN_BIT(s1)) // if they are both negative numbers then the compare sign has to be inverted at every further comparison
		compare = -compare;
