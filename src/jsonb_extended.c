...
{
	if( is_prefix_of_dstring(&get_dstring_pointing_to_cstring(dti_p->type_name), &get_dstring_pointing_to_literal_cstring(JSONB_TYPE_PREFIX)) &&
...
data_type_info* get_jsonb_extended_type_info(const char* extension_sub_type, uint32_t max_size, uint32_t inline_size, const page_access_specs* pas_p)
{
	...
	if(extension_sub_type != NULL)
		sprintf(dti_p->type_name, JSONB_TYPE_PREFIX "_{%s}_" EXTENDED_TYPE_SUFFIX, extension_sub_type);
