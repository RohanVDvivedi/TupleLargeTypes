...
struct binary_reader_interface
{
	...
	int (*is_valid)(const binary_reader_interface* bri_p);
	// call below functions only if the above one returns 1

	int (*is_null)(const binary_reader_interface* bri_p);
	// call below functions only if the above one returns 1

	uint32_t (*read_bytes_as_stream)(const binary_reader_interface* bri_p, char* data, uint32_t data_size, int* error);

	const char* (*peek_bytes_as_stream)(const binary_reader_interface* bri_p, uint32_t* data_size, int* error);

	// to be called on an error OR EOF OR when you do not need to read any more data
	void (*close_bytes_stream)(const binary_reader_interface* bri_p);
};
